...
# HERMES_DATA_MODE must be applied HERE, before the VOLUME declaration below:
# Docker discards any build-step write to a path after it is declared a volume,
# so a later `chmod /opt/data` (including from a derived stage) is silently
# voided. Default 0755 preserves upstream behaviour exactly; the Heroku build
# passes 0777 because Heroku runs the image as a random unprivileged UID that
# owns nothing and cannot be granted ownership at boot (no root). See the
# `heroku` stage at the end of this file.
ARG HERMES_DATA_MODE=0755
RUN mkdir -p /opt/data && chmod "${HERMES_DATA_MODE}" /opt/data
VOLUME [ "/opt/data" ]
...
# ---------- Heroku stage (reference deployment) ----------
# Heroku's container runtime ignores USER and runs the image as a random,
# unprivileged UID with no /etc/passwd entry, no root, and no volumes. The
# upstream boot path hard-rejects arbitrary non-root UIDs (stage2-hook.sh and
# main-wrapper.sh both exit 1), so this stage swaps in a Heroku-specific
# entrypoint that skips s6 supervision and all user-switching.
#
# This stage is intentionally LAST: Heroku's heroku.yml build cannot select a
# target stage and Docker builds the final stage by default. For the upstream-
# equivalent image, build with `docker build --target runtime .`.
#
# NOTE: /opt/data's permissions CANNOT be fixed in this stage — it is a
# declared VOLUME by now, so Docker discards writes to it during build. The
# build arg HERMES_DATA_MODE (applied before the VOLUME instruction above)
...
ENV HOME=/opt/data
ENTRYPOINT [ "/opt/hermes/docker/heroku-entrypoint.sh" ]
CMD [ ]
